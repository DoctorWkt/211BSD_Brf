...
#define BRF_OPTRUNCATE  20      // truncate(), not yet
#define BRF_OPFTRUNCATE 21      // ftruncate(), not yet
#define BRF_OPCREAT     22      // creat(), not yet
...
struct brf_open {	// BRF_OPOPEN data
  uint16_t flags;
  uint16_t mode;
};
...
struct brf_fchmod {	// fchmod() client request
  int16_t fd;		// File descriptor
  uint16_t mode;
};

struct brf_fchown {	// fchown() client request
  int16_t fd;		// File descriptor
