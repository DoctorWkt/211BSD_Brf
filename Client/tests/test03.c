...
#define FILENAME "/foo/dave"

char *mesg= "Another message ";
char *buf=  "                                              ";
...
int main(int argc, char *argv[]) {
  ...
  fd2= dup2(fd, 130);
  if (fd2 == -1) { perror("Dup2 failed"); exit(1); }
  printf("Writing using second fd\n");
  result= write(fd2, mesg, strlen(mesg));
  if (result == -1) { perror("Write on fd2 failed"); exit(1); }

  printf("Closing the fds\n");
  result= close(fd);
  if (result == -1) { perror("Close failed"); exit(1); }
  result= close(fd2);
  if (result == -1) { perror("Close on fd2 failed"); exit(1); }
